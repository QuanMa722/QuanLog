...
class QuanLog:
    ...
    def _truncate_filename(self, filename):
        ...
        ext_width = sum(char_width * 2 if '\u4e00' <= c <= '\u9fff' else char_width for c in ext)
    ...
    def _init_ui(self):
        ...
        self.select_btn = ttkbs.Button(btn_frame, text="Files", command=self.select_files, bootstyle=PRIMARY, width=15)
        self.upload_btn = ttkbs.Button(btn_frame, text="Upload", command=self._upload_thread, bootstyle=SUCCESS,
        ...
        ttkbs.Label(search_frame, text="Search:", font=("Times New Roman", 10, "bold")).pack(side=LEFT, padx=(0, 2))
    ...
    def _is_valid_dir(self, path, name, length):
    ...
    def filter_file_list(self, event):
        ...
        for d in sorted(res.keys(), reverse=True):
            ...
            for fp in sorted(res[d]):
                self.file_tree.insert(node, END, text=self._truncate_filename(os.path.basename(fp)), values=(fp,))
    ...
    def _upload_worker(self):
        self.root.after(0, lambda: (setattr(self, 'is_working', True),
        ...
        if text:
            target = self.get_unique_file_path(os.path.dirname(self.text_save_path),
                                               os.path.basename(self.text_save_path), True)
        ...
        if self.selected_files:
            for src in self.selected_files:
                ...
                target = self.get_unique_file_path(self.date_path, fname,
                                                   fname.endswith('.txt') and re.match(r'^\d{8}\.txt$', fname))
                shutil.copy2(src, target)
                size = round(os.path.getsize(src) / 1024, 2)
                self.write_log("file", filename=os.path.basename(target), size=size)

        self.root.after(0, lambda: (self._load_files_thread(), self._reset_state(),
        ...
        for i in range(3):
            try:
                if os.path.exists(fp):
                    os.remove(fp)
                self.root.after(0, lambda: (self.file_tree.delete(tree_item), self.switch_preview("text"),
                                            self._load_files_thread()))
    ...
    def _video_worker(self, fp):
        ...
        mw, mh = self.preview_frame.winfo_width() - 20, self.preview_frame.winfo_height() - 10
